...
function setStateMessage(message) {
    var stateElement = document.querySelector('#state');
    if (stateElement) {
        stateElement.innerHTML = "<p style=\"color:#FFF;\">".concat(message, "</p>");
    }
}
...
function setXValue() {
    var xValue;
    var inputX = document.querySelector('#x-input');
...
function setYValue() {
    var yValue;
    var inputY = document.querySelector('#y-input');
    if (inputY.value === '') {
...
function createFormData(valX, valY, valR) {
...
function postDataToController(data) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch('controller', {
                            method: 'POST',
                            body: data,
                        })];
...
function handleFormSubmission(event) {
    if (setXValue() && setYValue() && setRValue()) {
        clearStateMessage();
        var formData = createFormData(x, y, r);
